...
export const AudioClip = ({url, title, author, creationDate, descr}) => {
    ...
    return (
    <div className={expand ? 'h-max my-4 bg-zinc-800 border-x-[1px] border-red-700 shadow-lg flex flex-col justify-center' : ' h-16 my-4 bg-zinc-800 overflow-hidden cursor-pointer border-x-[1px] border-red-700 shadow-lg flex flex-col justify-center'} 
    onClick={ expand ? () => null : () => setExpand(true)}>
        <p className={expand ? 'text-2xl font-bold mt-4 px-4' : 'text-2xl font-bold px-4'}>{title}</p>
        <iframe title={`SoundCloud Embed - ${title}-${url}`} className={expand ? 'py-4 px-4' : 'h-0'} width="100%" height="200" scrolling="no" frameBorder="no" allow="autoplay"
        ...
        <div className={expand ? 'my-2 flex justify-center items-center' : 'hidden'}>
